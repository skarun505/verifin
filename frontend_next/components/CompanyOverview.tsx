...
export default function CompanyOverview({ company }: CompanyOverviewProps) {
    ...
    const downloadPDF = async () => {
        ...
        try {
            ...
            pdf.setFontSize(18)
            pdf.text(data.name, 15, 55)
            pdf.setFontSize(12)
            pdf.text(`${data.ticker} • ${data.sector}`, 15, 62)

            // Price info
            pdf.setFontSize(16)
            pdf.text(`Price: ${data.price}`, 15, 75)
            pdf.setFontSize(12)
            const changeColor: [number, number, number] = data.change_pct?.startsWith('+') ? [34, 197, 94] : [239, 68, 68]
            pdf.setTextColor(...changeColor)
            pdf.text(`${data.change} (${data.change_pct})`, 15, 82)

            // Key metrics
            pdf.setTextColor(0, 0, 0)
            pdf.text('Key Metrics:', 15, 95)
            pdf.setFontSize(10)
            pdf.text(`Market Cap: ${data.marketCap}`, 15, 102)
            pdf.text(`P/E Ratio: ${data.pe_ratio}`, 80, 102)
            pdf.text(`52W High: ${data['52_week_high']}`, 15, 109)
            pdf.text(`52W Low: ${data['52_week_low']}`, 80, 109)

            // Description
            pdf.setFontSize(12)
            pdf.text('Company Description:', 15, 122)
            pdf.setFontSize(9)
            const splitDesc = pdf.splitTextToSize(data.description, pageWidth - 30)
            pdf.text(splitDesc, 15, 129)

            // Footer
            pdf.setFontSize(8)
            pdf.setTextColor(128, 128, 128)
            pdf.text(`Generated by VeriFin • ${new Date().toLocaleString()}`, 15, 280)
            pdf.text(`Currency: ${data.currency} • Powered by Yahoo Finance`, 15, 285)

            // Save PDF
            pdf.save(`${data.name.replace(/[^a-z0-9]/gi, '_')}_VeriFin_Report.pdf`)
        } catch (error) {
                ...
                <div className="glass rounded-2xl p-6 border border-red-500/50">
                    ...
                    {error.includes('private') && (
                        <p className="text-gray-400 text-sm">
                            💡 Tip: Private companies don't have public stock data. Try public companies like MRF, Airtel, TCS, Apple, Tesla.
                        </p>
                    )}
                </div>
            )}
